...
public interface ProjectRepository extends JpaRepository<Project, UUID> {
    ...
    @Query("SELECT p FROM Project p WHERE p.archivedAt IS NULL AND (p.company.id = :companyId OR p.id IN :memberProjectIds) ORDER BY p.createdAt DESC")
    Page<Project> findAllVisibleToCustomer(@Param("companyId") UUID companyId, @Param("memberProjectIds") List<UUID> memberProjectIds, Pageable pageable);
